...
import {PopUpMessage,PopUpMessageWithAutoCancel} from '../AllFiles'
   ...
   const cancelMultipleCourse=()=>{
    ...
    const addSingleCourse=async(item)=>{
        ...
        // make a post request to the backend
            ...
            if((data?.status)) displayPopUpMessage(data?.message,setpopUpMsg,setpopUpType,setdisplayPopUp,data?.status)
            else  displayPopUpMessage(data?.code==='ERR_BAD_REQUEST' ? 'unable to add course, an error occured please try again later': data?.message ,setpopUpMsg,setpopUpType,setdisplayPopUp,data?.status)
    ...
    const handleCheckInput=(e,item)=>{
        ...
        setallRefs([0])  // redundant but necessary code to make the page re render
    }
    ...
    const addMultipleCourse=async()=>{
        ...
        // make a post request to the backend
            ...
            let data=await response
            if((data?.status)) displayPopUpMessage(data?.message,setpopUpMsg,setpopUpType,setdisplayPopUp,data?.status)
            else  displayPopUpMessage(data?.code==='ERR_BAD_REQUEST' ? 'unable to add courses, an error occured please try again later': data?.message ,setpopUpMsg,setpopUpType,setdisplayPopUp,data?.status)
            cancelPopUP(setdisplayPopUp,500)
            setcheckAllInput(false)
            setcheckAll(false)
    }
